...
const UsuariosTable = () => {
  const eliminarUsuario = async (_id, nombre) => {
    if (
      window.confirm(`📣 ¿Estás segur@ que querés 𝗘𝗟𝗜𝗠𝗜𝗡𝗔𝗥 a ${nombre}? 😮`)
    ) {
      await axios.delete(
  ...
  const traerUsuarios = async () => {
    try {
      const info = await axios.get(
        "https://laquiaquenaherboristeriabe.onrender.com/traerusuarios"
      );
      ...
      {usuariosOrganizados.map((usuario) => (
        <div className="card col-3 text-center mx-1 mb-3 card-usuarios">
          <div className="card-body">
            <h4 className="card-title title-pedido">
          ...
          <div className="d-flex align-items-center justify-content-around">
            <button
              ...
              onClick={(_id, nombre) =>
                eliminarUsuario(usuario._id, usuario.nombre)
              }
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
