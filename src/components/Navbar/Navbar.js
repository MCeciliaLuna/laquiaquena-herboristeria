...
const Navbar = () => {
  ...
  const cerrarSesion = () => {
    if (
      window.confirm(
        '¿Segur@ que quieres cerrar sesión del panel de ADMINSTRACIÓN?',
      )
    ) {
      window.location.shref = '/login';
    }
  };
  return (
      ...
      <nav className="navbar navbar-expand-lg">
        {isHomePage && (
          <svg
            ...
            className="salir-icon text-light pb-0 ms-3 my-3 bi bi-x-circle-fill"
            viewBox="0 0 16 16"
          >
