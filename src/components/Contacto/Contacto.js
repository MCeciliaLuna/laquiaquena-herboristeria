...
const Contacto = () => {
  return (
    ...
    <form className="iconos-redes p-3 w-50" action="https://formsubmit.co/lunama.cecilia@gmail.com" method="POST" >
      <label className="d-block fs-5 text-light">Email de Contacto</label>
      <input className="input-contacto rounded-3 p-2 border-0 d-block w-100 fs-5" type="email" name="email" pattern="^[^@]+@[^@]+\.[a-zA-Z]{2,}$" maxLength="35" required />
      ...
      <textarea className="input-contacto rounded-3 p-2 border-0 d-block w-100 fs-5" type="text" name="mensaje" rows="5" maxLength="500" required ></textarea>
      <div className="d-flex justify-content-center w-100">
      <button className="btn my-2 px-5 text-light" type="submit">Enviar</button>
      </div>
     </form>
     </div>
 </div>
